...
- **键盘快捷键**：Space 播放/暂停、Esc 取消选择、Delete 删除选中、Ctrl+F 搜索、方向键快进快退/调音量、M 静音（设置页面有完整快捷键表）
- **实时搜索**：按文件名和格式快速筛选，带清除按钮
- **自定义收藏夹**：创建自定义文件夹分类管理音效，点击星标弹出面板选择收藏到哪个文件夹
- **批量操作**：多选文件，批量收藏、取消收藏、从列表移除
- **设置页面**：默认音量调节、深色/浅色主题切换、数据清除
...
4. **搜索**：在顶部搜索栏输入关键词，按文件名或格式实时筛选
5. **播放**：点击音频卡片或在右侧播放面板控制播放/暂停/停止
6. **收藏到文件夹**：鼠标悬停音频卡片，点击五角星图标弹出收藏夹选择面板，选择目标文件夹或新建
7. **批量操作**：点击音频卡片左侧复选框进入批量模式，可全选、批量收藏、批量移除
8. **查看收藏**：点击左侧「我的收藏」查看所有已收藏音效
9. **音量调节**：右侧播放器面板底部拖动音量滑块，或点击喇叭图标一键静音
10. **设置**：点击左下角齿轮图标进入设置页面，调整默认音量、切换深色/浅色主题、查看快捷键表或清除所有数据
11. **键盘快捷键**：`Space` 播放/暂停、`Esc` 取消选择/停止播放、`Delete` 删除选中文件、`Ctrl+F` 聚焦搜索、`←→` 快进快退 5 秒、`↑↓` 音量增减 5%、`M` 静音切换
