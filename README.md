...
### v1.0.1 (2026-05-27) — 第六次更新

- **新增**：音效排序功能，支持按名称、文件大小、时长、格式排序，点击同字段切换升降序，排序偏好持久保存
- **改进**：设置页面抖音/B站链接改为使用系统默认浏览器打开，而非应用内窗口
- **调整**：移除侧边栏分类与收藏夹右侧的编辑图标，仅保留右键菜单操作（重命名/删除）

### v1.0.1 (2026-05-26) — 第五次更新

- **新增**：最近播放记录，自动追踪最近 20 首播放过的音效，侧边栏「最近播放」入口按顺序浏览
- **新增**：自定义键盘快捷键，设置页面点击快捷键按钮即可重新绑定，支持 Ctrl/Shift/Alt 组合键
- **新增**：设置页面增加作者信息卡片，含头像、邮箱、抖音、B 站链接
- **修复**：批量操作工具栏在子目录视图下「全选」未正确过滤文件

### v1.0.1 (2026-05-26) — 第二次更新

- **新增**：音效卡片右键菜单，支持播放、添加到收藏夹、在资源管理器打开、复制路径、重命名（仅软件内）、从列表移除
- **新增**：侧边栏子目录树形浏览，递归扫描后可按子文件夹层级浏览音效
- **新增**：时长显示增加「时长：」前缀，明确标注时间含义
- **修复**：无收藏夹时新建收藏夹再取消，不再意外触发音频播放

### v1.0.1 (2026-05-26) — 第三次更新

- **修复**：右键菜单关闭或操作时不再意外触发音频播放

### v1.0.1 (2026-05-26) — 第四次更新

- **修复**：Ctrl+A 全选快捷键现已正常工作
- **修复**：切换曲目时音量不再重置为 100%
- **修复**：「从列表移除」后重启软件，文件不再重新出现（通过 ignoredPaths 持久化）
- **修复**：AIFF、OPUS、WEBM、MKA 等 5 种格式现已拥有独立颜色标签
- **重构**：清理死代码（移除未使用的 store actions、IPC 通道、状态字段）
- **改进**：星标按钮改为快速收藏/取消收藏，无需弹窗；收藏夹管理移至右键菜单
- **改进**：侧边栏分类和收藏夹旁显示文件数量
- **新增**：批量操作工具栏增加「添加到收藏夹」按钮

### v1.0.1 (2026-05-26) — 初次发布

- **修复**：音效卡片布局优化为三行显示（名称 / 格式+大小 / 时长），避免文字排版错乱
- **修复**：点击收藏夹选择面板的关闭按钮（X）不再意外触发音频播放
- **修复**：收藏夹选择面板新增「完成」确认按钮，选择收藏夹后点击即可关闭
- **修复**：删除收藏夹后，该收藏夹内的文件现在可以正常取消收藏
- **修复**：无收藏夹时新建收藏夹再取消，不再意外触发音频播放
