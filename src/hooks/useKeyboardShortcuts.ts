...
export function useKeyboardShortcuts() {
  // Only read stable config and action references in the render scope
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (matches('deselect') && !editing) {
        ...
        if (state.selectedFileIds.size > 0) {
          deselectAllFiles();
        } else if (state.player.currentFile && state.player.isPlaying) {
          audioManager.stop();
          stopPlayback();
        }
        return;
      ...
      // Delete — Batch delete via store action (includes playback stop guard)
      if (matches('delete')) {
        const state = useStore.getState();
        if (state.selectedFileIds.size > 0) {
          e.preventDefault();
          if (window.confirm(
            `确定要删除已选中的 ${state.selectedFileIds.size} 个文件吗？（不会删除磁盘上的原始文件）`
          )) {
            batchDeleteFiles();
          }
        }
      ...
      if (matches('queueNext')) {
        e.preventDefault();
        const { selectedFileIds } = useStore.getState();
  ...
  }, [shortcuts, deselectAllFiles, stopPlayback, setVolume, addToPlayQueue, batchDeleteFiles]);
}

export function shortcutEventToString(e: KeyboardEvent): string {
  return keyCodeToChinese(e.code);
}

export function keyCodeToChinese(code: string): string {
...
export function shortcutToChinese(shortcut: string): string {
  const parts = shortcut.split('+');
  const modifiers: string[] = [];
  let key = '';
  for (const part of parts) {
  ...
  const chineseKey = keyCodeToChinese(key);
  if (modifiers.length > 0) {
    return [...modifiers, chineseKey].join(' + ');
  }
  return chineseKey;
}
