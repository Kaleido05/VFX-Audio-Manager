...
export const useStore = create<AppState>((set, get) => ({
      ...
      return {
        ...
        audioFiles: state.audioFiles.map((f) => {
          ...
          return {
            ...
            // Preserve explicit favorite flag; it's independent of collection membership
          };
        }),
      };
    });
  ...
  addToCollection: async (fileId: string, collectionId: string) => {
    ...
    try {
      await window.electronAPI.saveCollectionFiles(get().collectionFiles);
      // Persist file path to favorites to keep the star active
      const file = get().audioFiles.find((f) => f.id === fileId);
      if (file) {
        await window.electronAPI.addFavorite(file.path);
        set((state) => ({
          audioFiles: state.audioFiles.map((f) =>
            f.id === fileId ? { ...f, isFavorite: true } : f
          ),
        }));
      }
    } catch (err) {
  ...
  removeFromCollection: async (fileId: string, collectionId: string) => {
      ...
      return {
        ...
        audioFiles: state.audioFiles.map((f) =>
          f.id === fileId
            ? {
                ...f,
                isFavorite: f.isFavorite, // preserve explicit favorite
    ...
    try {
      await window.electronAPI.saveCollectionFiles(get().collectionFiles);
      // If file was only in this collection and not explicitly favorited, clean up
      const file = get().audioFiles.find((f) => f.id === fileId);
      const stillInAny = file?.collectionIds.length || 0;
      if (file && stillInAny === 0) {
        // No longer in any collection — remove from persisted favorites only if not star-faved
        // (star-faved files have explicit isFavorite not tied to collections)
      }
    } catch (err) {
    ...
    try {
      ...
      const defaultVolume =
        typeof raw.defaultVolume === 'number'
          ...
          : 0.8;
      const theme = (raw.theme === 'dark' || raw.theme === 'light' || raw.theme === 'warm' || raw.theme === 'forest' || raw.theme === 'ocean') ? raw.theme : 'dark';
      ...
      document.documentElement.classList.remove('light', 'warm', 'forest', 'ocean');
  ...
  updateSettings: async (update) => {
    set((state) => {
      ...
      if (update.theme) {
        document.documentElement.classList.remove('light', 'warm', 'forest', 'ocean');
        if (update.theme !== 'dark') document.documentElement.classList.add(update.theme);
      }
      return {
        settings: next,
  ...
  batchDeleteFiles: () => {
    const { selectedFileIds, audioFiles, player } = get();
    // Stop playback if the currently playing file is being deleted
    const removedIds = new Set(selectedFileIds);
    if (player.currentFile && removedIds.has(player.currentFile.id)) {
      audioManager.stop();
      set({ player: { ...initialPlayerState, volume: player.volume } });
    }
