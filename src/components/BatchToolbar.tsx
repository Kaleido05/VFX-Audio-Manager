...
export default function BatchToolbar() {
  ...
  // Click-away for batch collection picker
  useEffect(() => {
    if (!showBatchCollectionPicker) return;
  ...
  const visibleFiles = useMemo(() => {
    ...
    let favoritesFirstApplied = true;
    if (activeView.type === 'favorites') {
      ...
      // Don't apply regular sort/sortAsc for recentlyPlayed; recency order is the sort
      favoritesFirstApplied = false;
    }
    if (searchQuery) {
    ...
    return files;
  }, [audioFiles, activeView, searchQuery, sortKey, sortAsc, recentlyPlayedIds]);
  ...
  const handleToggleAll = () => {
    if (allSelected) {
      // Deselect only visible files, keep non-visible selections
      ...
      {userCollections.length > 0 && (
        <div className="relative">
          ...
          {showBatchCollectionPicker && (
            <div
              ref={batchPickerRef}
              className="absolute bottom-full left-0 z-50 mb-1 w-44 rounded-lg border border-surface-600 bg-surface-800 py-1 shadow-xl"
            >
