...
export default function Sidebar({ width }: SidebarProps) {
  ...
  // Click-away for context menus: close when clicking outside the menu
  useEffect(() => {
  ...
  // Build a directory tree from subPaths
  interface DirNode {
    name: string;
    fullPath: string;
    children: DirNode[];
  ...
  const dirTreeByCategory = useMemo(() => {
    const map = new Map<string, DirNode[]>();
    for (const file of audioFiles) {
      if (!file.subPath) continue;
      const segments = file.subPath.split('/');
      const catId = file.categoryId;
      if (!map.has(catId)) map.set(catId, []);
      const roots = map.get(catId)!;
      // Walk the tree and insert missing nodes
      ...
      for (const seg of segments) {
        ...
        if (!existing) {
          existing = { name: seg, fullPath: accumulated, children: [] };
    ...
    const sortTree = (nodes: DirNode[]) => {
      nodes.sort((a, b) => a.name.localeCompare(b.name));
      nodes.forEach((n) => sortTree(n.children));
    };
    for (const roots of map.values()) {
      sortTree(roots);
    }
    return map;
  }, [audioFiles]);

  const handleToggleDir = useCallback((e: React.MouseEvent, fullPath: string) => {
    e.stopPropagation();
  ...
  return (
    <aside className="flex shrink-0 flex-col border-r border-surface-700 bg-surface-900" style={{ width }}>
      {/* App logo / title */}
      ...
      <nav className="flex-1 overflow-y-auto px-3">
        ...
        <button
          ...
          className={`mb-0.5 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-[13px] transition-all ${
            isActive({ type: 'all' })
              ? 'bg-surface-700 text-primary'
              : 'text-surface-400 hover:bg-surface-800 hover:text-primary'
          }`}
        >
          <HiMusicalNote className="h-3.5 w-3.5 shrink-0" />
          全部音效
        </button>
        ...
        >
          <HiStar className="h-3.5 w-3.5 shrink-0" />
          我的收藏
        </button>
              ...
              <div key={cat.id} className="relative">
                {editingId === cat.id ? (
                  ...
                  <button
                    ...
                    className={`group/cat mb-0.5 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-[13px] transition-all ${
                  ...
                  >
                    ...
                    {(dirTreeByCategory.get(cat.id) || []).length > 0 && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                ...
                {expandedCategories.has(cat.id) && (
                  <DirTree
                    nodes={dirTreeByCategory.get(cat.id) || []}
                    categoryId={cat.id}
                ...
                {contextMenuId === cat.id && (
                  <div
                    ref={contextMenuRef}
                    className="absolute left-16 top-full z-20 mt-1 w-32 rounded-lg border border-surface-600 bg-surface-800 py-1 shadow-xl"
                  >
                      <button
          ...
          <>
            ...
            {userCollections.map((col) => (
              <div key={col.id} className="relative">
                {editingId === col.id ? (
                  <div className="mb-0.5 flex items-center gap-2 rounded-lg bg-surface-700 px-3 py-1.5">
                    <HiBookmark className="h-5 w-5 shrink-0 text-amber-400" />
                    <input
                      className="min-w-0 flex-1 rounded border border-accent-600 bg-surface-800 px-1.5 py-0.5 text-sm text-primary outline-none"
                      value={editName}
                      onChange={(e) => setEditName(e.target.value)}
                  ...
                  <button
                    ...
                    className={`group/col mb-0.5 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-[13px] transition-all ${
...
interface DirTreeProps {
  ...
  onToggleDir: (e: React.MouseEvent, fullPath: string) => void;
  isActive: (view: { type: 'subdirectory'; categoryId: string; subPath: string }) => boolean;
  setActiveView: (view: { type: 'subdirectory'; categoryId: string; subPath: string }) => void;
  depth: number;
}

function DirTree({ nodes, categoryId, expandedDirs, onToggleDir, isActive, setActiveView, depth }: DirTreeProps) {
  ...
  return (
    <>
      {nodes.map((node) => {
        ...
        const isNodeExpanded = expandedDirs.has(node.fullPath);
        return (
          <div key={node.fullPath}>
            <button
              onClick={() => setActiveView({ type: 'subdirectory', categoryId, subPath: node.fullPath })}
              className={`mb-0.5 flex w-full items-center gap-2 rounded-lg py-1.5 pr-3 text-[11px] transition-all ${
                isActive({ type: 'subdirectory', categoryId, subPath: node.fullPath })
                  ? 'bg-surface-700 text-primary'
                  : 'text-surface-400 hover:bg-surface-800 hover:text-primary'
              }`}
              style={{ paddingLeft }}
              title={node.fullPath}
            >
              <HiFolder className="h-3 w-3 shrink-0" />
              <span className="min-w-0 flex-1 truncate text-left">{node.name}</span>
              {hasChildren && (
                <button
                  onClick={(e) => onToggleDir(e, node.fullPath)}
                  className="shrink-0 rounded p-0.5 text-surface-500 hover:text-primary"
                >
                  {isNodeExpanded ? (
                    <HiChevronDown className="h-3 w-3" />
                  ) : (
